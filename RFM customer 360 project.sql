...
select *,
case 
	when recency >= min(recency) and recency < (select recency from customer_rn where rn_recency = (select max(rn_recency)*0.25 from customer_rn)) then 1
...
case 
	when frequency >= min(frequency) and frequency < (select frequency from customer_rn where rn_frequency = (select max(rn_frequency)*0.25 from customer_rn)) then 1
...
case 
	when monetary >= min(monetary) and monetary < (select monetary from customer_rn where rn_monetary = (select max(rn_monetary)*0.25 from customer_rn)) then 1
...
select *, 
	concat(r, f, m) as customer_grade,
case 
